...
// LoopbackServer briefly listens on 127.0.0.1 to catch the provider's OAuth
...
func StartLoopback() (*LoopbackServer, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("oauth2pkce: start loopback listener: %w", err)
	}
...
func (ls *LoopbackServer) handleRedirect(w http.ResponseWriter, r *http.Request) {
	...
	if errStr := q.Get("error"); errStr != "" {
		ls.resultCh <- LoopbackResult{Err: fmt.Errorf("access denied: %s", errStr)}
	} else {
