...
// The remedy table used to live here. It is in internal/remedy now, and
// reaches this app as `remedy` on a run: the browser was the only thing that
// had it, so `nanobots run` printed a bare error for a locked vault or a
// missing model and left a terminal user to work it out. Keeping a second
// copy here is exactly how the two would have drifted.
//
// What stays is the parsing above, because it is display formatting: list
// rows shorten an error client-side, and shipping a parsed field for every
// run in the list would cost more than it saves.
