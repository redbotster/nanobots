...
**`1claw`** — the previous behaviour, now one option among several. Key/value only, so it does not implement `Recaller` and `memory.recall` against it fails with a clear message rather than returning nothing.

That is a measured limit, not an assumed one, and it took a wrong turn to establish. 1Claw *does* expose `POST /v1/agents/{id}/memory/search` with a `top_k` and a score per result — the prose docs say memory search does not exist, the OpenAPI spec says it does, and the spec is normally the one to trust. Probed before wiring it up: an entry was stored and listed back, then searched for by exact text and matched nothing, while an empty query returned it. Any non-empty query returns zero results, and waiting a minute changes nothing. The route answers; the matching does not work. So this backend says it cannot recall rather than answering "nothing known" forever, which is the failure a bot could not tell from an empty memory (`docs/1claw-feature-requests.md` #12).
