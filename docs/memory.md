...
That flipped once already and is worth being precise about, because both states were measured, not assumed (`docs/1claw-feature-requests.md` #12). First probe: an entry was stored and listed back, then searched for by exact text and matched nothing, while an empty query returned everything. That is 1Claw's own bug, not this build's — the route answered, the matching didn't — so this backend deliberately did not implement `Recaller`. Re-probed after 1Claw shipped a fix: exact and partial-word queries now score and rank real matches (`{"query":"refunds"}` against a stored "refunds always get escalated to a human" scores `0.95`). What still doesn't work: a paraphrase sharing no words with the stored text (`"what happens with refund requests"`) matches nothing. So `Recall` here is real but **lexical** — it finds text that shares words with the question, not text that means the same thing — and it answers by handing back the matching stored text verbatim, not a synthesized sentence the way Honcho's dialectic answer is. `Remember` stores each observation under a generated key, since (unlike `Put`) noting something down has no key of its own.

An entry can carry a TTL — `ttl_seconds` on the write, `ttl_expires_at` on the entry, both verified live. Nothing here uses it yet, because no bot has scratch state that should expire. Entries hang off an agent id, which only exists once a run reaches that bot, so startup leaves a `DeferredOneClaw` marker that the runner swaps for a real store per bot. Within an agent, the namespace is the bot's own name, so two bots sharing an agent do not share memories.
