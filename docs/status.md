...
- **An idle tab costs almost nothing.** `GET /api/runs` was 91KB polled every
  two seconds and near-always identical. It carries an ETag and answers 304
  with no body; the client holds the tag and returns early, so there is no
  parse and no re-render either. Measured in a browser: 7 polls over 14
  seconds went from 638KB to 2.1KB.
...
| Approvals — a run blocks, flips to `awaiting_approval`, and waits for a real decision from the WebUI, the CLI, or 1Claw's own queue on your phone; first answer wins ([approvals.md](approvals.md)) | A local answer leaves the mirrored 1Claw approval pending, since its API has no cancel ([1claw-feature-requests.md](1claw-feature-requests.md)) |
