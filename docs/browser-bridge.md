...
Per `docs/connections.md`, Browser Bridge is the default `connection` strategy for any service with no viable OAuth or static-token path. It's real, working infrastructure in this build — just not exercised by any bot's `service.call` yet. Gmail/Drive don't use it because Google specifically blocks the pattern (see below); Stripe, HubSpot, X, and LinkedIn didn't end up needing it either — Stripe/HubSpot have long-lived static tokens simple enough to just paste into a vault secret, and X/LinkedIn have normal OAuth2 flows of their own (`internal/x`, `internal/linkedin`, both built on the shared `internal/oauth2pkce` core) — see `docs/connections.md` for all of them. What's left that still needs it: Google Business Profile replies (`review-responder`), which stays on `connection: demo` until it's wired up — Browser Bridge remains the fallback if that ever isn't worth building as a Google-OAuth scope extension instead (see the note in `docs/connections.md`).
